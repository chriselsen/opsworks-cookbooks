...
node[:deploy].each do |app_name, deploy|
	...
	execute "Import Wordpress database backup" do
		Chef::Log.debug("Importing Wordpress database backup: if ls #{deploy[:deploy_to]}/current/*.sql &> /dev/null; then #{mysql_command} < #{deploy[:deploy_to]}/current/*.sql; fi; exit 0")
